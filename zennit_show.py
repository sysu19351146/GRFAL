# -*- coding: utf-8 -*-
"""
Created on Sun Mar 12 22:01:41 2023
...
# relevance_format='symmetric_{sample}.png'
...
#         imsave(fname, relevance[n], vmin=0., vmax=1., level=1.0, cmap="coldnhot")
...
# @click.command()
# @click.argument('dataset-root', type=click.Path(file_okay=False),default='dataset_zennit')
# @click.argument('relevance_format', type=click.Path(dir_okay=False, writable=True),default='symmetric_{sample}.png')
...
def zennit_show(
    dataset_root="dataset_zennit",
    relevance_format='zen/symmetric3_{sample}.png',
    attributor_name='gradient',
    composite_name="epsilon_gamma_box",
    model_name=None,
    parameters=None,
    input_format='zen/input_{sample}.png',
    batch_size=20,
    max_samples=20,
    n_outputs=2,
    cpu=False,
    ...
    #norm_fn = BatchNormalize((0.5,0.5,0.5), (0.5,0.5,0.5), device=device)
    norm_fn = BatchNormalize([0.5,0.5,0.5], [0.5,0.5,0.5], device=device)
    ...
    with attributor:
        for data, target,_ in loader:
            ...
            for n in range(len(data)):
                fname = relevance_format.format(sample=sample_index + n)+".png"
                # zennit.image.imsave will create an appropriate heatmap given a cmap specification
            ...
            # update the accuracy
            accuracy += (output.argmax(1) == target.cuda()).sum().detach().cpu().item()
