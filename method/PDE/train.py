    ...
    for epoch in range(epoch_offset, config.n_epochs):
        ...
        if epoch%10==0:
            for split in additional_splits:
                test_results, y_pred = run_epoch(algorithm, datasets[split], general_logger, epoch, config, train=False)
                save_pred_if_needed(y_pred, datasets[split], epoch, config, is_best)
                curr_val_metric = test_results[config.val_metric]
                print(time.time() - kk)
                general_logger.write(f'test {config.val_metric}: {curr_val_metric:.3f}\n')
