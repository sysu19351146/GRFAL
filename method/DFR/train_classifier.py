...
from utils import MultiTaskHead,trades_train
...
for epoch in range(args.num_epochs):
    ...
    for batch in tqdm.tqdm(train_loader):
        ...
        loss = trades_train(model,x,y,optimizer,device)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
    ...
    if epoch % args.eval_freq == 0:
        ...
        for test_name, test_loader in test_loader_dict.items():
            results = evaluate(model, test_loader, get_yp_func, args.multitask, args.predict_place,device=device)
            results_pgd = evaluate(model, test_loader, get_yp_func, args.multitask, args.predict_place,device=device)
