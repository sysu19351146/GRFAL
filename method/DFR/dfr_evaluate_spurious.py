...
from utils import Logger, AverageMeter, set_seed, evaluate, get_y_p,evaluate_pgd,pgd_test,AA_test
...
base_model_results["test"] = evaluate_pgd(model, test_loader, get_yp_func,device=device)
base_model_results["val"] = evaluate_pgd(model, val_loader, get_yp_func,device=device)
base_model_results["train"] = evaluate_pgd(model, train_loader, get_yp_func,device=device)
...
for name, loader in [("train", train_loader), ("test", test_loader), ("val", val_loader)]:
    ...
    for x, y, g, p in tqdm.tqdm(loader):
        with torch.no_grad():
            #all_embeddings[name].append(get_embed(model, pgd_test(model,x.to(device),y.to(device),epsilon=0.00196,random_init=0.0001,device=device)).detach().cpu().numpy())
            all_embeddings[name].append(get_embed(model, x.to(device)).detach().cpu().numpy())
